##Use this script for Creating New Group and adding to mentioned SSO groups from WO.

##GroupName,GroupDescription,Owners,Members,SSOGroups  --All in CSV file with semicolon separated.
...
ForEach ($CSVRecord in $CSVRecords) {
    ...
    $SSOGroups        = if ($CSVRecord."SSOGroups") { $CSVRecord."SSOGroups" -split ';' } else { $null }
    ...
    Try {
        ...
        if ($ExistingGroup) {
            ...
            # Add new group to SSO groups
            if ($SSOGroups) {
                foreach ($SSOGroup in $SSOGroups) {
                    $SSOGroupObj = Get-AzureADGroup -Filter "DisplayName eq '$SSOGroup'" -ErrorAction SilentlyContinue
                    if ($SSOGroupObj) {
                        Try {
                            Add-AzureADGroupMember -ObjectId $SSOGroupObj.ObjectId -RefObjectId $NewGroup.ObjectId
                            Write-Host "Added '$GroupName' to SSO group '$SSOGroup'." -ForegroundColor Cyan
                        }
                        Catch {
                            Write-Host "Failed to add '$GroupName' to SSO group '$SSOGroup': $_" -ForegroundColor Red
                        }
                    }
                    else {
                        Write-Host "SSO group '$SSOGroup' not found." -ForegroundColor Yellow
                    }
                }
            }
